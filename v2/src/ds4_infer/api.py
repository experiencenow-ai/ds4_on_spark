...
class CoordinatorApi:
    def __init__(
        ...
        *,
        ...
        self._dispatcher_stats: dict[str, Any] = {
            "format": "ds4-dispatcher-status-v1",
            "state": "stopped",
            "worker_id": "spark0-api-background-dispatcher",
            "iterations": 0,
            "worked_iterations": 0,
            "claimed_count": 0,
            "completed_count": 0,
    ...
    def _handle_openai_completion(self, body: dict[str, Any]) -> tuple[int, dict[str, Any]]:
        ...
        if isinstance(prompt, list):
            return self._handle_openai_completion_prompt_array(body, registry, topology, profile, prompt)
    ...
    def _handle_openai_completion_prompt_array(self, body: dict[str, Any], registry: ProfileRegistry, topology: SparkTopology, profile: ModelProfile, prompts: list[Any]) -> tuple[int, dict[str, Any]]:
        if not prompts:
            return 400, {"error": "prompt array must not be empty"}
        base_id = str(body.get("request_id") or f"cmpl-{uuid.uuid4().hex}")
        batch_id = str(body.get("batch_id") or base_id)
        request_ids = body.get("request_ids")
        if request_ids is not None:
            if not isinstance(request_ids, list) or len(request_ids) != len(prompts):
                return 400, {"error": "request_ids must be an array with the same length as prompt"}
            ids = [str(value) for value in request_ids]
        else:
            ids = [f"{base_id}-{index:06d}" for index in range(len(prompts))]
        raw_requests = [
        ...
        submitted = self.queue.submit_requests(requests=[InferenceRequest.from_json(raw) for raw in raw_requests], registry=registry, topology=topology, batch_id=batch_id, priority=_optional_int(body.get("priority")))
        if _is_async_request(body):
            payload = _async_queue_response("openai_completion", base_id, batch_id, submitted)
            payload["request_ids"] = ids
            return 202, payload
        collected = self._run_batch_until_collected(batch_id=batch_id, timeout_s=float(body.get("ds4_timeout_s") or self.sync_timeout_s))
        return 200, _openai_completion_batch_response(request_id=base_id, model=str(body.get("model") or profile.model_id), collected=collected)
    ...
    def _run_batch_until_collected(self, *, batch_id: str, timeout_s: float) -> dict[str, Any]:
        ...
        while time.time() < deadline:
            ...
            if not self.dispatcher_running:
                self._work_once({"batch_id": batch_id})
            time.sleep(idle_sleep)
        return {"format": "ds4-inference-queue-v1", "batch_id": batch_id, "results": [], "error": "coordinator sync timeout"}

    @property
    def dispatcher_running(self) -> bool:
        thread = self._dispatcher_thread
        return bool(thread is not None and thread.is_alive() and not self._dispatcher_stop.is_set())
    ...
    def start_background_dispatcher(self) -> None:
        with self._dispatcher_lock:
            ...
            self._dispatcher_stats.update({"state": "starting", "updated_at": time.time()})
            self._dispatcher_thread = threading.Thread(target=self._dispatcher_loop, name="ds4-api-dispatcher", daemon=True)
            self._dispatcher_thread.start()

    def stop_background_dispatcher(self, timeout_s: float = 5.0) -> None:
        self._dispatcher_stop.set()
        thread = self._dispatcher_thread
        if thread is not None:
            thread.join(timeout=max(0.0, timeout_s))
        with self._dispatcher_lock:
            self._dispatcher_stats.update({"state": "stopped", "updated_at": time.time()})

    def dispatcher_status(self) -> dict[str, Any]:
        with self._dispatcher_lock:
            status = dict(self._dispatcher_stats)
        status["running"] = self.dispatcher_running
        return status

    def _dispatcher_loop(self) -> None:
        worker_id = os.environ.get("DS4_API_DISPATCH_WORKER_ID", "spark0-api-background-dispatcher")
        window = _env_int("DS4_API_DISPATCH_WINDOW", 64)
        refill_batch = _env_int("DS4_API_DISPATCH_REFILL_BATCH", 16)
        batch_linger_s = _env_float("DS4_API_DISPATCH_BATCH_LINGER_S", 0.01)
        idle_sleep_s = _env_float("DS4_API_DISPATCH_IDLE_SLEEP_S", 0.005)
        heartbeat_s = _env_float("DS4_API_DISPATCH_HEARTBEAT_S", 2.0)
        lease_ttl_s = _env_int("DS4_API_DISPATCH_LEASE_TTL_S", 900)
        timeout_s = _env_int("DS4_API_DISPATCH_REQUEST_TIMEOUT_S", int(self.sync_timeout_s))
        max_attempts = _env_int("DS4_API_TRANSPORT_MAX_ATTEMPTS", 3)
        body = {
            "limit": max(1, refill_batch),
            "concurrency": max(1, window),
            "batch_linger_s": max(0.0, batch_linger_s),
            "worker_id": worker_id,
            "lease_ttl_s": max(1, lease_ttl_s),
            "heartbeat_interval_s": max(0.1, heartbeat_s),
            "timeout_s": max(1, timeout_s),
            "transport_max_attempts": max(1, max_attempts),
        }
        with self._dispatcher_lock:
            self._dispatcher_stats.update({"state": "running", "worker_id": worker_id, "config": dict(body), "updated_at": time.time()})
        while not self._dispatcher_stop.is_set():
            try:
                worked = self._work_once(body)
                claimed = int(worked.get("claimed_count") or 0)
                with self._dispatcher_lock:
                    self._dispatcher_stats["iterations"] = int(self._dispatcher_stats.get("iterations") or 0) + 1
                    self._dispatcher_stats["claimed_count"] = int(self._dispatcher_stats.get("claimed_count") or 0) + claimed
                    self._dispatcher_stats["completed_count"] = int(self._dispatcher_stats.get("completed_count") or 0) + int(worked.get("completed_count") or 0)
                    self._dispatcher_stats["failed_count"] = int(self._dispatcher_stats.get("failed_count") or 0) + int(worked.get("failed_count") or 0)
                    self._dispatcher_stats["last_work"] = worked
                    self._dispatcher_stats["last_error"] = None
                    self._dispatcher_stats["state"] = "running"
                    self._dispatcher_stats["updated_at"] = time.time()
                    if claimed:
                        self._dispatcher_stats["worked_iterations"] = int(self._dispatcher_stats.get("worked_iterations") or 0) + 1
                if claimed <= 0:
                    self._dispatcher_stop.wait(max(0.001, idle_sleep_s))
            except Exception as exc:
                with self._dispatcher_lock:
                    self._dispatcher_stats.update({"state": "error", "last_error": str(exc), "updated_at": time.time()})
                self._dispatcher_stop.wait(max(0.05, idle_sleep_s))
...
def serve(*, host: str, port: int, queue_dir: str | Path, profiles_dir: str | Path, topology_path: str | Path, runner_kind: str = "pipeline") -> None:
    ...
    if _env_bool("DS4_API_BACKGROUND_DISPATCH", runner_kind != "fake"):
...
def _openai_completion_batch_response(*, request_id: str, model: str, collected: dict[str, Any]) -> dict[str, Any]:
    rows = collected.get("results")
    ...
    return {
        ...
        "ds4": {"batch_id": collected.get("batch_id"), "status_counts": statuses},
    }
...
def _batch_limits_by_service(topology: SparkTopology) -> dict[str, int]:
    ...
    raw = os.environ.get("DS4_API_BATCH_LIMITS_JSON")
    if not raw:
        return limits
    try:
        parsed = json.loads(raw)
    except json.JSONDecodeError as exc:
        raise ValueError(f"DS4_API_BATCH_LIMITS_JSON is not valid JSON: {exc}") from exc
    if not isinstance(parsed, dict):
        raise ValueError("DS4_API_BATCH_LIMITS_JSON must be a JSON object")
