...
class InferenceQueue:
    ...
    def claim_ready_batch(self, *, node_id: str | None, batch_id: str | None, limit: int, leased_by: str, lease_ttl_s: int, batch_linger_s: float = 0.0, kv_shard_layouts_by_profile: Mapping[str, Any] | None = None, batch_limits_by_service: Mapping[str, int] | None = None) -> list[QueueClaim]:
        now = time.time()
        with closing(self._connect()) as conn, conn:
            rows = _ready_rows(conn, node_id=node_id, batch_id=batch_id, limit=limit, batch_limits_by_service=batch_limits_by_service or {})
            if not rows:
                return []
            ...
            if len(rows) < linger_limit and batch_linger_s > 0:
                ...
                if (now - newest_ready) < batch_linger_s:
                    return []
            compute_lease_id = self._acquire_compute_lease(conn, rows=rows, leased_by=leased_by, lease_ttl_s=lease_ttl_s, now=now)
            if compute_lease_id is False:
                return []
            ...
            for row in rows:
                ...
                updated = conn.execute(
                    ...
                    """,
                    (lease_id, compute_lease_id if isinstance(compute_lease_id, str) else None, leased_by, now + lease_ttl_s, now, now, now, row["request_id"]),
                ).rowcount
                if updated == 1:
                    batch_ids.add(str(row["batch_id"]))
                    self._event(conn, str(row["request_id"]), "started", "running", {"batch_id": row["batch_id"], "lease_id": lease_id, "node_id": row["selected_node_id"], "service_id": row["selected_service_id"], "compute_domain": row["selected_compute_domain"], "compute_lease_id": compute_lease_id if isinstance(compute_lease_id, str) else None})
                    claims.append(_claim(row, lease_id, compute_lease_id if isinstance(compute_lease_id, str) else None))
            if not claims and isinstance(compute_lease_id, str):
...
def _ready_rows(conn: sqlite3.Connection, *, node_id: str | None, batch_id: str | None, limit: int, batch_limits_by_service: Mapping[str, int] | None = None) -> list[sqlite3.Row]:
    clauses = ["state='ready'"]
    params: list[Any] = []
    if node_id is not None:
