"""Striped TCP payload channel for DS4 pipeline-parallel tensor dicts."""
...
class Ds4TcpTensorHandle:
    """Thread-backed handle matching vLLM's small async handle protocol."""
...
class Ds4TcpTensorChannel:
    ...
    def can_handle(self, tensor: torch.Tensor) -> bool:
        if not self._envs.VLLM_DS4_PP_TCP_TENSOR_DICT:
            return False
        if tensor.numel() <= 0:
            return False
        byte_size = tensor.numel() * tensor.element_size()
        return byte_size >= int(self._envs.VLLM_DS4_PP_TCP_MIN_BYTES)
    ...
    def recv(self, tensor: torch.Tensor, src: int) -> Ds4TcpTensorHandle:
        ...
        ports: list[int] = []
        host = (
            self._envs.VLLM_DS4_PP_TCP_BIND_HOST
            or self._envs.VLLM_HOST_IP
            or "0.0.0.0"
        )
        ...
        for index, (start, end) in enumerate(ranges):
            ...
            thread.start()
        advertised_host = (
            self._envs.VLLM_DS4_PP_TCP_ADVERTISE_HOST or self._envs.VLLM_HOST_IP
        )
    ...
    def _send_stripe(
        ...
        try:
            timeout = float(self._envs.VLLM_DS4_PP_TCP_CONNECT_TIMEOUT_SECONDS)
            with socket.create_connection((host, port), timeout=timeout) as sock:
                if self._envs.VLLM_DS4_PP_TCP_NODELAY:
    ...
    def _recv_stripe(
        ...
        try:
            ...
            with conn:
                ...
                conn.settimeout(float(self._envs.VLLM_DS4_PP_TCP_READ_TIMEOUT_SECONDS))
                offset = 0
