...
def _parse_args(argv: list[str] | None) -> argparse.Namespace:
    ...
    parser.add_argument("--fanout-nodes", help="comma-separated destination nodes for source-rooted ring fanout")
    ...
    if args.fanout_nodes is not None and not args.fanout_all:
        parser.error("--fanout-nodes requires --fanout-all")
    return args
...
def _selected_stages(topology: TransferTopology, args: argparse.Namespace) -> list[list[tuple[str, str]]]:
    ...
    destinations = _fanout_destinations(topology, args.source_node, args.fanout_nodes)
    return _source_rooted_ring_stages(topology, args.source_node, destinations)


def _fanout_destinations(topology: TransferTopology, source_node: str, fanout_nodes: str | None) -> list[str]:
    topology.get_node(source_node)
    if fanout_nodes is None:
        destinations = [node_id for node_id in topology.nodes if node_id != source_node]
    else:
        destinations = [item.strip() for item in fanout_nodes.split(",") if item.strip() != ""]
    if not destinations:
        raise ValueError("fanout requires at least one destination node")
    seen: set[str] = set()
    for node_id in destinations:
        topology.get_node(node_id)
        if node_id == source_node:
            raise ValueError("fanout destinations must not include source node")
        if node_id in seen:
            raise ValueError(f"duplicate fanout destination node: {node_id}")
        seen.add(node_id)
    return destinations


def _source_rooted_ring_stages(topology: TransferTopology, source_node: str, destinations: list[str]) -> list[list[tuple[str, str]]]:
    ring = sorted([source_node, *destinations], key=_node_sort_key)
    source_index = ring.index(source_node)
    ordered = ring[source_index:] + ring[:source_index]
    stages: list[list[tuple[str, str]]] = []
    previous = source_node
    for node in ordered[1:]:
        stages.append([(previous, node)])
        previous = node
    return stages
