...
class PersistentSimpleOffloadStore:
    ...
    def from_env(
        ...
        *,
        ...
        if not root:
            return None
        rank_key = os.getenv(RANK_ENV) or os.getenv("VLLM_HOST_IP") or socket.gethostname()
        store = cls(
            root=Path(root),
    ...
    def load_scheduler_entries(self, num_cpu_blocks: int) -> list[PersistedBlock]:
        return self._entries_from_index(self._read_json(self.scheduler_index), num_cpu_blocks)

    def load_worker_entries(self, num_cpu_blocks: int) -> list[PersistedBlock]:
        return self._entries_from_index(self._read_json(self.worker_index), num_cpu_blocks)

    def save_scheduler_blocks(self, cpu_block_ids: list[int], block_hashes: list[str]) -> None:
    ...
    def persist_worker_blocks(
        ...
        for cpu_block_id, hash_hex in zip(cpu_block_ids, block_hashes):
            try:
                ...
                self._fail(f"failed to persist CPU offload block {_short_hash(hash_hex)}", exc)
    ...
    def restore_worker_blocks(
        ...
        for entry in self.load_worker_entries(num_cpu_blocks):
            ...
            if not path.exists():
                self._fail(f"persistent CPU offload block missing: {_short_hash(entry.hash_hex)} at {path}")
                continue
            try:
                ...
                for name, dst in cpu_kv_caches.items():
                    ...
                    if tuple(src.shape) != tuple(target.shape):
                        raise ValueError(f"tensor {name} shape mismatch {tuple(src.shape)} != {tuple(target.shape)}")
                    target.copy_(src)
                restored[entry.cpu_block_id] = entry.hash_hex
            except Exception as exc:
                self._fail(f"failed to restore CPU offload block {_short_hash(entry.hash_hex)}", exc)
        return restored
        ...
        for cpu_block_id, hash_hex in zip(cpu_block_ids, block_hashes):
            ...
            if actual != hash_hex:
                self._fail(
                    ...
                    f"for cpu_block={cpu_block_id} hash={_short_hash(hash_hex)} actual={_short_hash(actual)}"
                )

    def _entries_from_index(self, data: dict[str, Any] | None, num_cpu_blocks: int) -> list[PersistedBlock]:
        if not data:
            return []
        if int(data.get("version", 0)) != FORMAT_VERSION:
            self._fail(f"unsupported persistent offload index version in {data.get('format')}")
            return []
        if data.get("model") not in (None, self.model_key):
            self._fail(f"persistent offload model mismatch: {data.get('model')} != {self.model_key}")
            return []
        entries: list[PersistedBlock] = []
        seen_cpu_ids: set[int] = set()
        for hash_hex, raw in sorted((data.get("blocks") or {}).items(), key=lambda item: int(item[1].get("cpu_block_id", -1))):
            try:
                ...
                if cpu_block_id in seen_cpu_ids:
                    continue
                entries.append(PersistedBlock(cpu_block_id=cpu_block_id, hash_hex=hash_hex, block_hash=bytes.fromhex(hash_hex)))
                seen_cpu_ids.add(cpu_block_id)
            except Exception as exc:
                self._fail(f"invalid persistent offload index entry {hash_hex}", exc)
        return entries

    def _upsert_index(self, path: Path, cpu_block_ids: list[int], block_hashes: list[str]) -> None:
        self._validate_pairs(cpu_block_ids, block_hashes)
        ...
        for cpu_block_id, hash_hex in zip(cpu_block_ids, block_hashes):
            ...
            for old_hash, old_entry in list(blocks.items()):
                if int(old_entry.get("cpu_block_id", -1)) == cpu_id and old_hash != hash_hex:
                    blocks.pop(old_hash, None)
            blocks[hash_hex] = {"cpu_block_id": cpu_id, "updated_at": time.time()}
        data["updated_at"] = time.time()
        self._write_json_atomic(path, data)

    def _validate_pairs(self, cpu_block_ids: list[int], block_hashes: list[str]) -> None:
        if len(cpu_block_ids) != len(block_hashes):
            self._fail(f"CPU block/hash length mismatch {len(cpu_block_ids)} != {len(block_hashes)}")
        for cpu_block_id, hash_hex in zip(cpu_block_ids, block_hashes):
    ...
    def _fail(self, message: str, exc: Exception | None = None) -> None:
        ...
        logger.warning("DS4 persistent SimpleCPUOffload warning: %s", message, exc_info=exc is not None)


def _model_key(vllm_config: Any) -> str:
    model_config = getattr(vllm_config, "model_config", None)
    model = getattr(model_config, "model", None) or getattr(model_config, "served_model_name", None)
