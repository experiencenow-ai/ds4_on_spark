...
QWEN_PP_DEPLOYMENT = ROOT / "profiles" / "kv_cache" / "qwen27_nvfp4_pp8_lmcache_hma.json"
...
class KvCachePlanningTests(unittest.TestCase):
    ...
    def test_qwen_lmcache_mp_plan_uses_external_cache_server(self) -> None:
        ...
        self.assertIn("/home/spark7/ds4_lmcache/qwen27/l2", plan["cache_server"]["command"])
    ...
    def test_write_qwen_lmcache_launch_scripts(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("mkdir -p /home/spark7/ds4_lmcache/qwen27/l2", install)
            self.assertIn("lmcache server", cache_server)
            self.assertIn("LMCacheMPConnector", vllm)

    def test_qwen_nvfp4_pp8_lmcache_hma_plan_is_pipeline_sharded(self) -> None:
        deployment = KvCacheDeployment.load(QWEN_PP_DEPLOYMENT)
        plan = plan_deployment(deployment)

        self.assertEqual(plan["profile_id"], "qwen3_6_27b_nvfp4_pp8_fast_cache_v1")
        ...
        self.assertIn("--quantization", plan["vllm_nodes"][0]["argv"])
        self.assertEqual(plan["vllm_nodes"][0]["argv"][plan["vllm_nodes"][0]["argv"].index("--quantization") + 1], "modelopt")
        self.assertIn("--linear-backend", plan["vllm_nodes"][0]["argv"])
        self.assertEqual(plan["vllm_nodes"][0]["argv"][plan["vllm_nodes"][0]["argv"].index("--linear-backend") + 1], "flashinfer-cutlass")
    ...
    def test_kv_cache_is_optional_on_existing_profiles(self) -> None:
        ...
        pp_qwen_fast = registry.get("qwen3_6_27b_nvfp4_pp8_fast_cache_v1")
        ...
        self.assertEqual(pp_qwen_fast.routing["optional_kv_cache_deployments"], ["profiles/kv_cache/qwen27_nvfp4_pp8_lmcache_hma.json"])
        ...
        self.assertEqual(registry.resolve(capability="fast", chat=False, job_class="summary").profile_id, pp_qwen_fast.profile_id)
