...
class KvCachePlanningTests(unittest.TestCase):
    ...
    def test_kimi27_pp13_lmcache_plan_uses_uniform_local_directory_and_source_guard(self) -> None:
        ...
        self.assertEqual(rank0["argv"][rank0["argv"].index("--gpu-memory-utilization") + 1], "0.09")
        self.assertEqual(rank0["argv"][rank0["argv"].index("--kv-cache-memory-bytes") + 1], "2147483648")
    ...
    def test_glm52_pp13_direct_plan_fills_pipeline(self) -> None:
        ...
        self.assertEqual(rank0["argv"][rank0["argv"].index("--max-num-seqs") + 1], "32")
        self.assertEqual(rank0["argv"][rank0["argv"].index("--max-num-batched-tokens") + 1], "16384")
        ...
        self.assertIn("--enforce-eager", rank0["argv"])
        self.assertEqual(rank0["argv"][rank0["argv"].index("--kv-cache-dtype") + 1], "fp8_e4m3")
        self.assertEqual(rank0["argv"][rank0["argv"].index("--attention-backend") + 1], "TRITON_MLA")
        self.assertEqual(rank0["env"]["VLLM_DS4_SCHED_MAX_NEW_REQS_PER_STEP"], "32")
        self.assertEqual(rank12["env"]["VLLM_DS4_SCHED_MAX_NEW_REQS_PER_STEP"], "32")
        self.assertEqual(rank0["env"]["VLLM_DS4_PP_OVERLAP_SEND"], "1")
        self.assertEqual(rank0["env"]["VLLM_DS4_PP_SEND_BUFFER_SLOTS"], "8")
        ...
        self.assertEqual(rank0["env"]["DS4_VLLM_SM12_SPARSE_INDEXER_DENSE_FALLBACK"], "1")
    ...
    def test_write_gemma26_lmcache_scripts_materialize_config(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("mkdir -p /home/spark4/ds4_nvme/ds4_lmcache/gemma4_26b_a4b_pp8_bf16kv/p3_4_4_4_3_4_4_4", install)
    ...
    def test_dsv4_flash_pp8_simple_offload_plan_is_pipeline_sharded(self) -> None:
        ...
        self.assertEqual(
            plan["connector"]["kv_transfer_config"]["kv_connector_extra_config"]["cpu_bytes_to_use_per_rank"],
            "3221225472",
        )
        ...
        self.assertEqual(plan["vllm_nodes"][0]["env"]["VLLM_SIMPLE_KV_OFFLOAD_PERSIST_ROOT"], "/home/spark0/ds4_nvme/ds4_hma_store/dsv4_flash_pp8/simple_cpu_offload/p4_5_4_7_5_5_7_6")
        self.assertEqual(plan["vllm_nodes"][-1]["env"]["VLLM_SIMPLE_KV_OFFLOAD_PERSIST_ROOT"], "/home/spark7/ds4_nvme/ds4_hma_store/dsv4_flash_pp8/simple_cpu_offload/p4_5_4_7_5_5_7_6")
