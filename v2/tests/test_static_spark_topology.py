...
class StaticSparkTopologyTests(unittest.TestCase):
    def test_kimi_qwen_gemma_pp13_topology_uses_all_thirteen_sparks(self) -> None:
        ...
        self.assertEqual(kimi.scheduler["refill_low_watermark"], 64)
        self.assertEqual(qwen.scheduler["refill_low_watermark"], 48)
        ...
        self.assertEqual(kimi.kv_cache["gpu_memory_utilization"], 0.09)
        self.assertEqual(qwen.kv_cache["gpu_memory_utilization"], 0.09)
        self.assertEqual(gemma.kv_cache["gpu_memory_utilization"], 0.142)
        for service in (kimi, qwen, gemma):
        ...
        self.assertEqual(coordinator["dispatch_window"], 448)
        self.assertEqual(coordinator["dispatch_refill_batch"], 448)
        self.assertEqual(coordinator["dispatch_cohort_workers"], 448)
    ...
    def test_kimi27_pp13_topology_is_dedicated_qualification_service(self) -> None:
        ...
        self.assertEqual(topology.routing_policy["resident_coordinator_defaults"]["completion_token_budget"], 524288)
    ...
    def test_glm52_dedicated_topology_separates_long_context_from_batch_width(self) -> None:
        ...
        self.assertEqual(glm.scheduler["initial_cohort_min"], 80)
        ...
        self.assertEqual(coordinator["completion_cohort_max"], 80)
        self.assertEqual(coordinator["completion_pp_safe_cohort_max"], 80)
        self.assertEqual(coordinator["dispatch_window"], 80)
        self.assertEqual(coordinator["dispatch_refill_batch"], 80)
        self.assertEqual(coordinator["dispatch_cohort_workers"], 80)
        self.assertTrue(coordinator["prefer_cohort_batch"])
        ...
        self.assertEqual(plans["glm52_fp8_pp13"].queue_depth_target, 80)
        self.assertEqual(plans["glm52_fp8_pp13"].initial_cohort_min, 80)
    ...
    def test_kimi27_dedicated_readiness_has_gpu_budget(self) -> None:
        ...
        try:
            ...
            self.assertEqual(payload["resident_gpu_memory_utilization"], {"kimi27_pp13": 0.09})
            self.assertEqual(payload["resident_gpu_memory_utilization_sum"], 0.09)
    ...
    def test_kimi_qwen_readiness_counts_fixed_kv_bytes_as_headroom(self) -> None:
        ...
        self.assertEqual(payload["resident_gpu_memory_utilization"], {"kimi27_pp13": 0.09, "qwen27_bf16_pp13": 0.09})
        ...
        self.assertAlmostEqual(payload["resident_gpu_memory_utilization_sum"], 0.18)
        self.assertEqual(payload["resident_fixed_kv_cache_memory_bytes"], {"kimi27_pp13": 2147483648, "qwen27_bf16_pp13": 2147483648})
        self.assertEqual(payload["resident_fixed_kv_cache_memory_bytes_sum"], 4294967296)
        kimi = topology.pipeline_service_by_id("kimi27_pp13")
        self.assertEqual(kimi.scheduler["admission_mode"], "resident_multimodel_rolling_refill")
    ...
    def test_kimi_qwen_pp13_topology_drops_gemma_for_ram_headroom(self) -> None:
        ...
        self.assertEqual(coordinator["completion_token_budget"], 524288)
        self.assertEqual(coordinator["dispatch_kv_capacity_bytes"], 4294967296)
