...
QWEN_FAST_PP = "qwen3_6_27b_nvfp4_pp8_fast_cache_v1"
...
class StaticSparkTopologyTests(unittest.TestCase):
    def test_capacity_reflects_dual_pipeline_services(self) -> None:
        ...
        self.assertEqual(capacity[QWEN_FAST_PP], 48)
        self.assertEqual(capacity[QWEN_PP], 32)
    ...
    def test_pipeline_services_are_all_spark_and_spark0_ingress(self) -> None:
        topology = SparkTopology.load(TOPOLOGY)
        qwen_fast = topology.pipeline_service_by_id("qwen27_nvfp4_pp8")
    ...
    def test_run_manifest_records_pipeline_assignments(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(manifest["topology_id"], "static_sparks_2026_05_30_three_pp_v1")
    ...
    def test_xhigh_live_validation_manifest_lists_pipeline_checks(self) -> None:
        ...
        self.assertEqual(
            task_ids,
            [
                "xhv-001-qwen27-nvfp4-pp8-service",
                "xhv-002-qwen27-bf16-pp8-service",
                "xhv-003-dsv4-flash-pp8-service",
                "xhv-004-single-ingress-queue-submit",
                "xhv-005-shared-compute-domain-scheduler",
                "xhv-006-pipeline-kv-shards",
                "xhv-007-stage-telemetry",
                "xhv-008-non8-pipeline-config",
            ],
        )
        qwen_task = manifest["tasks"][0]
        self.assertEqual(qwen_task["target_nodes"], list(ALL_SPARKS))
        self.assertIn(QWEN_FAST_PP, qwen_task["profiles"])
    ...
    def test_startup_plan_marks_ingress_and_stage_items(self) -> None:
        ...
        self.assertEqual([item["action"] for item in spark0["items"]], ["pipeline_ingress_warm", "pipeline_ingress_warm", "pipeline_ingress_warm"])
        self.assertEqual([item["service_id"] for item in spark0["items"]], ["dsv4_flash_pp8", "qwen27_bf16_pp8", "qwen27_nvfp4_pp8"])
        self.assertEqual([item["action"] for item in spark4["items"]], ["pipeline_stage", "pipeline_stage", "pipeline_stage"])
        self.assertEqual(spark4["items"][0]["stage_index"], 4)
        self.assertEqual(spark4["items"][1]["layer_start"], 35)
        self.assertEqual([item["action"] for item in spark7["items"]], ["pipeline_stage", "pipeline_stage", "pipeline_stage"])
    ...
    def test_startup_warm_posts_only_ingress_items(self) -> None:
        ...
        self.assertEqual(result["warm_count"], 3)
        self.assertEqual(
            calls,
            [
                ...
                ("http://127.0.0.1:8103/v1/chat/completions", "sakamakismile/Qwen3.6-27B-Text-NVFP4-MTP"),
            ],
        )
