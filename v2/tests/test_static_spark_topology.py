...
class StaticSparkTopologyTests(unittest.TestCase):
    def test_kimi_qwen_gemma_pp13_topology_uses_all_thirteen_sparks(self) -> None:
        ...
        self.assertEqual(kimi.kv_cache["gpu_memory_utilization"], 0.262)
        self.assertEqual(qwen.kv_cache["gpu_memory_utilization"], 0.233)
        self.assertEqual(gemma.kv_cache["gpu_memory_utilization"], 0.146)
        for service in (kimi, qwen, gemma):
    ...
    def test_kimi27_dedicated_readiness_has_gpu_budget(self) -> None:
        ...
        try:
            ...
            self.assertEqual(payload["resident_gpu_memory_utilization"], {"kimi27_pp13": 0.262})
            self.assertEqual(payload["resident_gpu_memory_utilization_sum"], 0.262)
    ...
    def test_kimi_qwen_readiness_counts_fixed_kv_bytes_as_headroom(self) -> None:
        ...
        self.assertEqual(payload["resident_gpu_memory_utilization"], {"kimi27_pp13": 0.262, "qwen27_bf16_pp13": 0.233})
        ...
        self.assertAlmostEqual(payload["resident_gpu_memory_utilization_sum"], 0.495)
        self.assertEqual(payload["resident_fixed_kv_cache_memory_bytes"], {"kimi27_pp13": 6248464384, "qwen27_bf16_pp13": 7289700352})
        self.assertEqual(payload["resident_fixed_kv_cache_memory_bytes_sum"], 13538164736)
        kimi = topology.pipeline_service_by_id("kimi27_pp13")
        self.assertEqual(kimi.scheduler["admission_mode"], "resident_multimodel_rolling_refill")
