...
class CoordinatorApiKvCacheTests(unittest.TestCase):
    def test_openai_chat_shape_uses_spark0_queue(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(request["selected_service_id"], "qwen27_nvfp4_pp8")
    ...
    def test_external_kv_manifest_is_pipeline_sharded(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            code, prefetched = api.handle_post(
                ...
                {"namespace": "centaur.longmem", "kv_key": "doc:abc", "service_id": "qwen27_nvfp4_pp8"},
            )
    ...
    def test_external_kv_lease_pin_and_evict_rules(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            api = CoordinatorApi(queue_dir=tmp, profiles_dir=PROFILES, topology_path=TOPOLOGY, runner_kind="fake")
            api.handle_post("/ds4/kvcache/declare", {"kv_key": "stable-prefix", "service_id": "qwen27_nvfp4_pp8", "total_bytes": 80})
            code, leased = api.handle_post("/ds4/kvcache/lease", {"kv_key": "stable-prefix", "service_id": "qwen27_nvfp4_pp8", "owner": "diamondizer", "mode": "read"})
            self.assertEqual(code, 200)
            self.assertEqual(leased["lease"]["mode"], "read")
            code, pinned = api.handle_post("/ds4/kvcache/pin", {"kv_key": "stable-prefix", "service_id": "qwen27_nvfp4_pp8"})
            self.assertEqual(code, 200)
            self.assertEqual(pinned["pin_count"], 1)
            with self.assertRaises(ValueError):
                api.handle_post("/ds4/kvcache/evict", {"kv_key": "stable-prefix", "service_id": "qwen27_nvfp4_pp8"})
            code, released = api.handle_post("/ds4/kvcache/release", {"lease_id": leased["lease"]["lease_id"]})
            self.assertEqual(code, 200)
            self.assertTrue(released["released"])
            api.handle_post("/ds4/kvcache/unpin", {"kv_key": "stable-prefix", "service_id": "qwen27_nvfp4_pp8"})
            code, evicted = api.handle_post("/ds4/kvcache/evict", {"kv_key": "stable-prefix", "service_id": "qwen27_nvfp4_pp8"})
            self.assertEqual(code, 200)
    ...
    def test_external_kv_list_and_touch_for_centaur_memory(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            code, listed = api.handle_post(
                "/ds4/kvcache/list",
                {"namespace": "centaur.longmem", "service_id": "qwen27_nvfp4_pp8", "prefix": "wm:", "include_shards": True},
            )
            self.assertEqual(code, 200)
    ...
    def test_dsv4_scheduler_respects_pipeline_service_batch_limit(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            api = CoordinatorApi(queue_dir=tmp, profiles_dir=PROFILES, topology_path=TOPOLOGY, runner_kind="fake")
            ...
            worked = api._work_once({"batch_id": "dsv4-batch", "limit": 48, "concurrency": 48, "batch_linger_s": 0.0})
            self.assertEqual(worked["claimed_count"], 3)
            self.assertEqual(worked["completed_count"], 3)
            self.assertEqual(api.queue.status(batch_id="dsv4-batch")["completed_count"], 3)
