...
HISTORY_METRICS = [
    ...
    {"key": "req_s", "label": "REQ/S", "field": "vllm_requests_per_s", "fallback_field": "local_queue_completion_req_s", "unit": ""},
...
h1{font-size:22px;line-height:1.1;margin:0}.meta{color:var(--muted);text-align:right;line-height:1.5}.summary{display:grid;grid-template-columns:repeat(auto-fit,minmax(120px,1fr));gap:10px;margin-bottom:14px}
...
const metricModes={queue:["req_s","input_tok_s","output_tok_s","cache_tok_s","cache_hit_pct","vllm_running","vllm_waiting"],gpu:["gpu_pct","kv_pct","mem_pct","temp_c","power_w"]};
...
function card(n){let err=n.error||n.fetch_error||"";return `<article class="card ${n.state} ${n.node===selectedNode?"selected":""}" data-node="${n.node}"><header><div class="node">${n.node}</div><div class="pill">${n.state_label}</div></header><div class="bars">${bar("GPU",n.gpu_pct,"gpu")}${bar("KV",n.kv_pct,"kv",n.kv_known)}${bar("MEM",n.mem_pct,"mem")}</div><div class="details"><span>Req <b>${val(n.req_s)}</b></span><span>In <b>${val(n.input_tok_s)}</b></span><span>Out <b>${val(n.output_tok_s)}</b></span><span>Cache <b>${pct(n.cache_hit_pct)}</b></span><span>Ext <b>${pct(n.external_hit_pct)}</b></span><span>Run <b>${fmt(n.vllm_running)}/${fmt(n.vllm_waiting)}</b></span><span>Queue <b>${fmt(n.local_q_depth)}</b></span><span>Temp <b>${fmt(n.gpu_temp_c)}C</b></span></div>${err?`<div class="error">${err}</div>`:""}</article>`}
...
function metricScale(metric,points){if(["tok_s","input_tok_s","output_tok_s","cache_tok_s"].includes(metric.key))return Math.max(50,...points.map(p=>Number(p[metric.key])||0))*1.2;let fixed={gpu_pct:100,kv_pct:100,cpu_pct:100,mem_pct:100,temp_c:100,power_w:100,vllm_running:64,vllm_waiting:64,queue_depth:128,req_s:10,cache_hit_pct:100,external_hit_pct:100};return fixed[metric.key]||100}
...
async function refresh(){try{let r=await fetch("/api/summary",{cache:"no-store"});let d=await r.json();if(!selectedNode&&d.nodes&&d.nodes.length)selectedNode=d.nodes[0].node;document.getElementById("updated").textContent="updated "+(d.updated_iso||"unknown");document.getElementById("source").textContent=d.summary_path||"";document.getElementById("summary").innerHTML=[metric("Active",`${fmt(d.active_nodes)}/${d.reachable_nodes}`),metric("Run/Wait",`${fmt(d.vllm_running)}/${fmt(d.vllm_waiting)}`),metric("Req/s",val(d.req_s)),metric("In tok/s",val(d.input_tok_s)),metric("Out tok/s",val(d.output_tok_s)),metric("Cache hit",d.cache_known?pct(d.max_cache_hit_pct):"n/a"),metric("Queue Depth",fmt(d.queue_depth))].join("");document.getElementById("nodes").innerHTML=d.nodes.map(card).join("");wireCards();refreshHistory()}catch(e){document.getElementById("updated").textContent="dashboard read failed: "+e}}
...
def normalize_node(node: str, row: dict[str,Any], error_streak: int = NODE_DOWN_ERROR_THRESHOLD) -> dict[str,Any]:
    ...
    return({
        ...
        "req_s": max(fnum(row.get("last_vllm_requests_per_s")),fnum(row.get("last_local_queue_completion_req_s"))),
...
def build_snapshot(summary_path: str) -> dict[str,Any]:
    ...
    queue_req_s_by_node = node_metric_map(queue.get("local_queue_completion_req_s_by_node",""))
    ...
    for node in nodes:
        ...
        node["req_s"] = max(fnum(node.get("req_s")),queue_req_s_by_node.get(name,0.0))
        ...
        if node["state"] == "idle" and (fnum(node.get("req_s")) > 0.0 or fnum(node.get("input_tok_s")) > 0.0 or fnum(node.get("output_tok_s")) > 0.0 or fnum(node.get("vllm_running")) > 0.0):
            node["state"] = "busy"
    ...
    req_s = max(sum(fnum(node.get("req_s")) for node in reachable),fnum(queue.get("local_queue_completion_req_s",0.0)))
