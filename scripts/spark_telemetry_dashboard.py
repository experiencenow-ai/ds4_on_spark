...
<div class="chat-head"><div><div class="chat-title">DSAPI Chat</div><div class="label">manual model test console</div></div><div class="chat-controls"><select id="chat-model"><option value="glm52_fp8_pp13_frontier_v1">GLM 5.2</option><option value="kimi27_pp13">Kimi 2.7</option><option value="qwen27_bf16_pp13">Qwen27</option><option value="gemma4_26b_a4b_pp13">Gemma4</option></select><label>max <input id="chat-max" type="number" min="1" max="8192" step="1" value="512"></label><label>priority <input id="chat-priority" type="number" min="0" max="100" step="1" value="5"></label><label><input id="chat-stream" type="checkbox" checked>stream</label></div></div>
...
function chatPayload(messages=chatMessages,requestId=chatRequestId(),attachmentBytes=attachmentTotalBytes()){let max=Number(document.getElementById("chat-max").value)||512;let kept=messages.filter(m=>(m.role==="system"||m.role==="user"||m.role==="assistant")&&String(m.content||"").trim()!=="");return {request_id:requestId,batch_id:requestId,model:document.getElementById("chat-model").value,messages:kept,max_tokens:max,temperature:0,stream:document.getElementById("chat-stream").checked,priority:chatPriority(),ds4_timeout_s:1800,ds4_job_class:"interactive",metadata:{dashboard:"spark_telemetry_chat",context_message_count:kept.length,attachment_count:chatAttachments.length,attachment_bytes:attachmentBytes}}}
