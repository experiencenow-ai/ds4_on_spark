...
def node_state(row: dict[str,Any], error_streak: int = NODE_DOWN_ERROR_THRESHOLD) -> tuple[str,str]:
    ...
    if fnum(row.get("stale_data")) > 0.0 or (str(row.get("fetch_error","")) != "" and fnum(row.get("last_sample_age_s")) > STALE_SAMPLE_WARN_S):
        return("warn","stale")
...
def normalize_node(node: str, row: dict[str,Any], error_streak: int = NODE_DOWN_ERROR_THRESHOLD) -> dict[str,Any]:
    ...
    if state not in ("warn","down") and fnum(row.get("last_sample_age_s")) <= STALE_SAMPLE_WARN_S:
        fetch_error = ""
    return({
        "node": node,
        "state": state,
