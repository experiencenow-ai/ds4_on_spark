...
function chatPayload(messages=chatMessages){let max=Number(document.getElementById("chat-max").value)||512;return {model:document.getElementById("chat-model").value,messages:messages.filter(m=>(m.role==="system"||m.role==="user"||m.role==="assistant")&&String(m.content||"").trim()!==""),max_tokens:max,temperature:0,stream:document.getElementById("chat-stream").checked,priority:chatPriority(),ds4_timeout_s:1800,ds4_job_class:"interactive"}}
...
async function sendChat(){if(chatBusy)return;let input=document.getElementById("chat-input");let prompt=input.value.trim();if(!prompt&&!chatAttachments.length)return;chatBusy=true;document.getElementById("chat-send").disabled=true;let userContent=buildUserContent(prompt);let payload=chatPayload(chatMessages.concat([{role:"user",content:userContent}]));let requestBody=JSON.stringify(payload);let requestBytes=new TextEncoder().encode(requestBody).length;if(requestBytes>CHAT_MAX_BODY_BYTES){setChatStatus(`chat request is ${bytesLabel(requestBytes)}; dashboard limit is ${bytesLabel(CHAT_MAX_BODY_BYTES)}`);chatBusy=false;document.getElementById("chat-send").disabled=false;return}input.value="";clearChatFiles();chatMessages.push({role:"user",content:userContent});let assistantIndex=chatMessages.push({role:"assistant",content:""})-1;renderChat();let started=Date.now();setChatStatus("queued through DSAPI");try{let resp=await fetch("/api/chat/completions",{method:"POST",headers:{"Content-Type":"application/json"},body:requestBody});if(!resp.ok){let body=await resp.text();throw new Error(`HTTP ${resp.status}: ${body}`)}if(payload.stream&&resp.body){await readChatStream(resp,assistantIndex,started)}else{let data=await resp.json();let choice=(data.choices||[{}])[0]||{};let msg=choice.message||{};updateAssistant(assistantIndex,msg.content||"");setChatStatus(chatUsageLine(data.usage||{},started))}}catch(e){updateAssistant(assistantIndex,`[chat error] ${e}`);setChatStatus("failed")}finally{chatBusy=false;document.getElementById("chat-send").disabled=false}}
