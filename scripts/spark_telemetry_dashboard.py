...
CHAT_ATTACHMENT_MAX_TOTAL_BYTES = 768 * 1024
CHAT_DEFAULT_PRIORITY = 5
...
<div class="chat-head"><div><div class="chat-title">DSAPI Chat</div><div class="label">manual model test console</div></div><div class="chat-controls"><select id="chat-model"><option value="glm52_fp8_pp13_frontier_v1">GLM 5.2</option><option value="kimi27_pp13">Kimi 2.7</option><option value="qwen27_bf16_pp13">Qwen27</option><option value="gemma4_26b_a4b_pp13">Gemma4</option></select><label>max <input id="chat-max" type="number" min="1" max="131072" step="1" value="2048"></label><label>think <input id="chat-thinking" type="number" min="0" max="32768" step="1" value="0"></label><label>priority <input id="chat-priority" type="number" min="0" max="100" step="1" value="5"></label><label><input id="chat-stream" type="checkbox" checked>stream</label></div></div>
<div class="chat-body"><div class="chat-log" id="chat-log"><div class="empty">ask a model something</div></div><div class="chat-compose"><textarea id="chat-input" placeholder="Type a test prompt. Ctrl-Enter sends."></textarea><div class="chat-attach"><input id="chat-files" type="file" multiple><div class="chat-attach-note">Text attachments are included in the user turn. Limit: 5 files, 256 KiB each, 768 KiB total, 1 MiB request body.</div><div class="chat-files" id="chat-files-list"></div></div><div class="chat-actions"><button class="primary" id="chat-send">Send</button><button id="chat-reset">Reset Context</button></div><div class="chat-status" id="chat-status"></div></div></div>
</section>
...
const CHAT_ATTACHMENT_MAX_FILE_BYTES=256*1024;
const CHAT_ATTACHMENT_MAX_TOTAL_BYTES=768*1024;
const CHAT_DEFAULT_PRIORITY=5;
const CHAT_MAX_BODY_BYTES=1024*1024;
...
function bytesLabel(n){let x=Number(n)||0;if(x<1024)return `${x} B`;if(x<1024*1024)return `${(x/1024).toFixed(1)} KiB`;return `${(x/(1024*1024)).toFixed(2)} MiB`}
function attachmentTotalBytes(files=chatAttachments){return files.reduce((sum,file)=>sum+(Number(file.size)||0),0)}
function renderChatFiles(){let list=document.getElementById("chat-files-list");if(!list)return;if(!chatAttachments.length){list.innerHTML="";return}list.innerHTML=chatAttachments.map((file,index)=>`<div class="chat-file"><span class="chat-file-name">${esc(file.name)} (${bytesLabel(file.size)})</span><button type="button" onclick="removeChatFile(${index})">remove</button></div>`).join("")}
function clearChatFiles(){chatAttachments=[];let input=document.getElementById("chat-files");if(input)input.value="";renderChatFiles()}
function removeChatFile(index){chatAttachments=chatAttachments.filter((_,i)=>i!==index);renderChatFiles();setChatStatus(chatAttachments.length?`${chatAttachments.length} file(s) attached, ${bytesLabel(attachmentTotalBytes())}`:"")}
async function addChatFiles(fileList){let incoming=Array.from(fileList||[]);let next=chatAttachments.slice();for(let file of incoming){if(next.length>=CHAT_ATTACHMENT_MAX_FILES){setChatStatus(`attachment limit is ${CHAT_ATTACHMENT_MAX_FILES} files`);break}if(file.size>CHAT_ATTACHMENT_MAX_FILE_BYTES){setChatStatus(`${file.name} is ${bytesLabel(file.size)}; per-file limit is ${bytesLabel(CHAT_ATTACHMENT_MAX_FILE_BYTES)}`);continue}let text=await file.text();let size=new TextEncoder().encode(text).length;if(size>CHAT_ATTACHMENT_MAX_FILE_BYTES){setChatStatus(`${file.name} is ${bytesLabel(size)} after UTF-8 decode; per-file limit is ${bytesLabel(CHAT_ATTACHMENT_MAX_FILE_BYTES)}`);continue}let candidate=next.concat([{name:file.name,size:size,type:file.type||"text/plain",text:text}]);if(attachmentTotalBytes(candidate)>CHAT_ATTACHMENT_MAX_TOTAL_BYTES){setChatStatus(`attachment total limit is ${bytesLabel(CHAT_ATTACHMENT_MAX_TOTAL_BYTES)}`);continue}next=candidate}chatAttachments=next;let input=document.getElementById("chat-files");if(input)input.value="";renderChatFiles();if(chatAttachments.length)setChatStatus(`${chatAttachments.length} file(s) attached, ${bytesLabel(attachmentTotalBytes())}`)}
function formatAttachments(files){if(!files.length)return "";let parts=["\\n\\nAttached files. Treat these as user-provided files attached to this chat turn, and use their contents when answering."];files.forEach(file=>{parts.push(`\\n--- begin attached file: ${file.name} (${bytesLabel(file.size)}, ${file.type||"text/plain"}) ---\\n${file.text}\\n--- end attached file: ${file.name} ---`)});return parts.join("")}
function buildUserContent(prompt){return prompt+formatAttachments(chatAttachments)}
function chatPriority(){let priority=Number(document.getElementById("chat-priority").value);if(!Number.isFinite(priority))return CHAT_DEFAULT_PRIORITY;return Math.max(0,Math.min(100,Math.floor(priority)))}
function chatThinkingBudget(){let thinking=Number(document.getElementById("chat-thinking").value);if(!Number.isFinite(thinking))return 0;return Math.max(0,Math.min(32768,Math.floor(thinking)))}
function chatRequestId(){if(window.crypto&&crypto.randomUUID)return `chatcmpl-${crypto.randomUUID().replaceAll("-","")}`;return `chatcmpl-${Date.now().toString(16)}${Math.floor(Math.random()*1e9).toString(16)}`}
function chatPayload(messages=chatMessages,requestId=chatRequestId(),attachmentBytes=attachmentTotalBytes()){let max=Number(document.getElementById("chat-max").value)||512;let thinking=chatThinkingBudget();let kept=messages.filter(m=>(m.role==="system"||m.role==="user"||m.role==="assistant")&&String(m.content||"").trim()!=="");return {request_id:requestId,batch_id:requestId,model:document.getElementById("chat-model").value,messages:kept,max_tokens:max,thinking_budget_tokens:thinking,temperature:0,stream:document.getElementById("chat-stream").checked,priority:chatPriority(),ds4_timeout_s:1800,ds4_job_class:"interactive",metadata:{dashboard:"spark_telemetry_chat",context_message_count:kept.length,attachment_count:chatAttachments.length,attachment_bytes:attachmentBytes,thinking_budget_tokens:thinking}}}
function updateAssistant(index,text){chatMessages[index].content=text;renderChat()}
function chatDs4Line(ds4){let request=(ds4||{}).request||{};let rid=(ds4||{}).request_id||request.request_id||(ds4||{}).batch_id||"";let service=request.selected_service_id||"";let state=(ds4||{}).status||request.state||"";let parts=[];if(rid)parts.push(`request ${rid}`);if(service)parts.push(service);if(state)parts.push(state);return parts.join(" · ")}
function chatUsageLine(usage,started,ds4={}){let elapsed=(Date.now()-started)/1000;let c=Number((usage||{}).completion_tokens)||0;let p=Number((usage||{}).prompt_tokens)||0;let t=elapsed>0&&c>0?(c/elapsed).toFixed(2):"0.00";let trace=chatDs4Line(ds4);return `done in ${elapsed.toFixed(1)}s · prompt ${p} · output ${c} · ${t} tok/s${trace?` · ${trace}`:""}`}
async function readChatStream(resp,index,started){let reader=resp.body.getReader();let decoder=new TextDecoder();let buffer="";let text="";let usage={};let ds4={};while(true){let {value,done}=await reader.read();if(done)break;buffer+=decoder.decode(value,{stream:true});let parts=buffer.split("\\n\\n");buffer=parts.pop()||"";for(let part of parts){let lines=part.split("\\n").filter(l=>l.startsWith("data:")).map(l=>l.slice(5).trim());if(!lines.length)continue;let data=lines.join("\\n");if(data==="[DONE]")continue;let event=JSON.parse(data);let choice=(event.choices||[{}])[0]||{};let delta=choice.delta||{};if(delta.content){text+=delta.content;updateAssistant(index,text)}if(event.usage)usage=event.usage;if(event.ds4)ds4=event.ds4}}setChatStatus(chatUsageLine(usage,started,ds4))}
async function sendChat(){if(chatBusy)return;let input=document.getElementById("chat-input");let prompt=input.value.trim();if(!prompt&&!chatAttachments.length)return;chatBusy=true;document.getElementById("chat-send").disabled=true;let attachmentBytes=attachmentTotalBytes();let userContent=buildUserContent(prompt);let requestId=chatRequestId();let payload=chatPayload(chatMessages.concat([{role:"user",content:userContent}]),requestId,attachmentBytes);let requestBody=JSON.stringify(payload);let requestBytes=new TextEncoder().encode(requestBody).length;if(requestBytes>CHAT_MAX_BODY_BYTES){setChatStatus(`chat request is ${bytesLabel(requestBytes)}; dashboard limit is ${bytesLabel(CHAT_MAX_BODY_BYTES)}`);chatBusy=false;document.getElementById("chat-send").disabled=false;return}input.value="";clearChatFiles();chatMessages.push({role:"user",content:userContent});let assistantIndex=chatMessages.push({role:"assistant",content:""})-1;renderChat();let started=Date.now();setChatStatus(`queued through DSAPI · request ${requestId} · context ${payload.messages.length} message(s) · body ${bytesLabel(requestBytes)} · attachments ${bytesLabel(attachmentBytes)}`);try{let resp=await fetch("/api/chat/completions",{method:"POST",headers:{"Content-Type":"application/json"},body:requestBody});if(!resp.ok){let body=await resp.text();throw new Error(`HTTP ${resp.status}: ${body}`)}if(payload.stream&&resp.body){await readChatStream(resp,assistantIndex,started)}else{let data=await resp.json();let choice=(data.choices||[{}])[0]||{};let msg=choice.message||{};updateAssistant(assistantIndex,msg.content||"");setChatStatus(chatUsageLine(data.usage||{},started,data.ds4||{}))}}catch(e){updateAssistant(assistantIndex,`[chat error] ${e}`);setChatStatus(`failed · request ${requestId}`)}finally{chatBusy=false;document.getElementById("chat-send").disabled=false}}
function setupChat(){let send=document.getElementById("chat-send");let reset=document.getElementById("chat-reset");let input=document.getElementById("chat-input");let files=document.getElementById("chat-files");if(send)send.onclick=sendChat;if(reset)reset.onclick=()=>{chatMessages=[];clearChatFiles();renderChat();setChatStatus("context reset; next request starts from an empty chat history")};if(input)input.addEventListener("keydown",e=>{if(e.key==="Enter"&&e.ctrlKey){e.preventDefault();sendChat()}});if(files)files.addEventListener("change",e=>addChatFiles(e.target.files));renderChat();renderChatFiles()}
