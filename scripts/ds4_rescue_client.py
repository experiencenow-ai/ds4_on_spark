...
def main() -> int:
    ...
    ap.add_argument("command", choices=["health", "status", "ssh-probe", "restart-ssh", "self-rescue", "peer-rescue"])
    ...
    elif args.command == "self-rescue":
        out = request(args.host, args.port, token, "POST", "/action", {"action": "self_rescue"})
    else:
        out = request(args.host, args.port, token, "POST", "/action", {"action": "peer_rescue"})
    try:
