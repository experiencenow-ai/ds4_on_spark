...
def validate_profile(profile: dict[str,object]) -> None:
    ...
    if recovery.get("user") != "root" or recovery.get("port") != 2222:
        raise SpecNodeError("recovery SSH must be root public-key access on port 2222")
...
def parse_arguments(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--profile",type=Path,
        default=repo / "v2/profiles/auxiliary/rtx5090_speculation.json",
    )
...
def main(argv: list[str] | None = None) -> int:
    try:
        ...
        print(f"ds4_rtx5090_spec_node: {error}",file=sys.stderr)
        return(1)
