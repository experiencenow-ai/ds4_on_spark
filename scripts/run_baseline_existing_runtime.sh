...
fetch_remote_dir_tar()
{
    ...
    ssh $SSH_OPTS "$target" "sh -lc 'set -eu; dir=\"\$1\"; if [ ! -d \"\$dir\" ]; then exit 0; fi; base=\"\${dir##*/}\"; parent=\"\${dir%/*}\"; if [ \"\$parent\" = \"\$dir\" ] || [ \"\$parent\" = \"\" ]; then parent=\".\"; fi; tar -C \"\$parent\" -czf - \"\$base\"' sh $(sh_quote "$remote_dir")" >"$local_tgz" 2>"$local_tgz.stderr" || true
}
