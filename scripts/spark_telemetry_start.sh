...
for node in "${node_list[@]}"; do
	...
	if ssh "$node" 'systemctl --user daemon-reload && systemctl --user enable ds4-spark-telemetry.service && systemctl --user restart ds4-spark-telemetry.service'; then
		...
		ssh "$node" "pkill -f spark_node_telemetry_monitor.py || true; nohup python3 ~/bin/spark_node_telemetry_monitor.py --node \"\$(whoami)\" --interval 5 --out-dir '$telemetry_dir' > '$telemetry_dir/node_telemetry.nohup.log' 2>&1 & echo pid=\$!"
	fi
done
