...
def _run_lane_a_serial(module: Any, args: argparse.Namespace, requests: Sequence[PromptRequest]) -> tuple[SerialGeneration, ...]:
	session = _make_pipeline_session(module, args)
...
def _make_pipeline_session(module: Any, args: argparse.Namespace) -> Any:
	...
	stages = None
	if getattr(args, "stage_manifest", ""):
		load_stage_manifest = getattr(module, "load_stage_manifest", None)
		if load_stage_manifest is None:
			raise PipelineBatchSchedulerError("stage manifest requested but backend module has no load_stage_manifest")
		stages = load_stage_manifest(args.stage_manifest)
	return session_cls(stages=stages)
...
def build_parser() -> argparse.ArgumentParser:
	...
	ap.add_argument("--stage-manifest", default="", help="Optional scripts.pipeline_session stage manifest.")
