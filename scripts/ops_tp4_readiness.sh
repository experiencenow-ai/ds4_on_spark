...
{
    cat <<'EOF'
ops_tp4_readiness.sh -- safe DS4 TP=4 readiness checks (Spark0..Spark3)

Usage:
  ops_tp4_readiness.sh --self <name> [--topology ring|full] [--strict] [--hosts <h0,h1,h2,h3>] [--tcp <port>]... [--env <path>]...

Environment:
  SSH_OPTS            Optional ssh options override.
  DS4_RING_HOSTS      Optional comma-separated hosts in rank order: h0,h1,h2,h3.
  ...
  DS4_WORLD_SIZE      Optional; strict requires 4.
  DS4_RANK            Optional; strict requires 0..3 and used to pick ring neighbors.
...
strict_validate()
{
    ...
    if [ "${DS4_WORLD_SIZE:-}" = "" ]; then
        ...
        fail=1
    elif [ "$DS4_WORLD_SIZE" -ne 4 ]; then
        echo "strict: DS4_WORLD_SIZE must be 4 for TP=4: $DS4_WORLD_SIZE" >&2
        fail=1
    fi
    ...
    if [ "${DS4_RANK:-}" = "" ]; then
        ...
        fail=1
    elif [ "$DS4_RANK" -gt 3 ]; then
        echo "strict: DS4_RANK out of range for TP=4 (0..3): $DS4_RANK" >&2
        fail=1
    fi
    ...
    if [ "${DS4_MASTER_ADDR:-}" = "" ]; then
        ...
        case "${DS4_MASTER_ADDR:-}" in
            127.0.0.1|localhost|\:\:1|0.0.0.0|\:\:|\[\:\:\])
                echo "strict: DS4_MASTER_ADDR looks local/wildcard for TP=4: ${DS4_MASTER_ADDR}" >&2
                fail=1
                ;;
        esac
    fi
    ...
    if [ "$hosts_csv" = "" ] && [ "${DS4_RING_HOSTS:-}" = "" ]; then
        echo "strict: provide --hosts or DS4_RING_HOSTS (comma-separated h0,h1,h2,h3)" >&2
        fail=1
    else
        csv="$hosts_csv"
        if [ "$csv" = "" ]; then
        ...
        if [ "$#" -ne 4 ]; then
            echo "strict: invalid DS4_RING_HOSTS/--hosts (expected h0,h1,h2,h3): $csv" >&2
            fail=1
        else
            if ! hosts_unique "$1" "$2" "$3" "$4"; then
...
if [ "$hosts_csv" = "" ]; then
    ...
    if [ "$#" -ne 4 ]; then
        echo "ring hosts: invalid csv (expected h0,h1,h2,h3): $hosts_csv" >&2
        if [ "$strict" -ne 0 ]; then
            exit 2
        fi
    else
        ...
        echo "ring hosts: h0=$h0 h1=$h1 h2=$h2 h3=$h3"

        peers=""
        if [ "$topology" = "ring" ]; then
            set -- $(ring_neighbors "$h0" "$h1" "$h2" "$h3" 2>/dev/null || true)
            if [ "$#" -ne 2 ]; then
            ...
            self_host="$(self_host_from_rank "$h0" "$h1" "$h2" "$h3" 2>/dev/null || true)"
