...
{
	cat <<'USAGE'
Centaur v73 ring-step coordinator for Spark1/Spark2/Spark3 using rsync staging.

This runs on Spark0 (or any orchestrator host that can SSH to Spark1/2/3) and
works around the Centaur constraint that `hyor-ring-step` requires local,
writable peer roots. It does this by:

  1) Maintaining local working copies of node roots for spark0..spark3
  2) Running ring-step locally across those roots
  3) rsync'ing the mutated node roots back to Spark1/2/3
...
Usage:
  centaur_spark_ring_rsync_v73.sh <spark1_user@host> <spark2_user@host> <spark3_user@host> [remote_base_dir]
...
workdir="${RING_WORKDIR:-$HOME/centaur-smoke/v73/ring_rsync}"
...
remote_s3="$remote_base/hyor/node_spark3"
...
echo "== push mutated node roots back to Spark1/2/3 =="
