...
default_remote_dir="/tmp/ds4_cuda_probe_disasm_${remote_tag}"
...
printf "== cuda_probe_disasm_spark0 log: %s ==\n" "$(date -u +%Y-%m-%dT%H:%M:%SZ)" >> "$log_path"
tmp_out="$(mktemp "/private/tmp/ds4_cuda_probe_disasm_spark0_out.XXXXXX")"
set +e
main >"$tmp_out" 2>&1
rc=$?
set -e
