...
class SchedulerSimTest(unittest.TestCase):
    ...
    def test_runtime_trace_ablation_markdown_smoke(self) -> None:
        tmp_path = ""
        with tempfile.NamedTemporaryFile("w", delete=False) as f:
            tmp_path = f.name
            f.write(json.dumps({"t_ms": 0.0, "cls": "batch", "candidates": [0], "accepted_mtp": 1, "rejected_mtp": 1}))
            f.write("\n")
        try:
    ...
    def test_summary_emits_task_queue_wait_and_expert_queue_wait_percentiles(self) -> None:
        trace = [
            scheduler_sim.TokenRoute(t_ms=0.0, cls=scheduler_sim.LatencyClass.BATCH, candidates=(0,), k=1)
            for _ in range(4)
        ]
        ...
        self.assertAlmostEqual(float(s["expert_max_task_queue_wait_ms_p95"]), 3.0, places=6)
        self.assertAlmostEqual(float(s["expert_max_task_queue_wait_ms_max"]), 3.0, places=6)
        self.assertAlmostEqual(float(s["task_queue_wait_ms_p50_interactive"]), 0.0, places=6)
        self.assertAlmostEqual(float(s["task_queue_wait_ms_p95_interactive"]), 0.0, places=6)
    ...
    def test_mtp_draft_layer0_backpressure_clamps_accept_len_when_drafts_are_batch_queued(self) -> None:
        trace = [
            scheduler_sim.TokenRoute(
                t_ms=0.0,
                cls=scheduler_sim.LatencyClass.BATCH,
        ...
        cfg = scheduler_sim.SimConfig(
            ...
            mtp_draft_queue_cls="batch",
        )
        token_states: list[scheduler_sim.TokenState] = []
        m = scheduler_sim.run_simulation(cfg, trace, token_states_out=token_states)
        self.assertEqual(len(token_states), 2)
        self.assertTrue(token_states[1].mtp_draft_layer0_skipped_backpressure)
        self.assertTrue(token_states[1].mtp_accept_len_clamped_backpressure)
        self.assertEqual(token_states[1].mtp_draft_attempt_len, 0)
        self.assertEqual(token_states[1].mtp_accept_len, 1)
        self.assertEqual(token_states[1].output_len, 1)
        self.assertFalse(token_states[1].mtp_verify_layer0_skipped_backpressure)
        self.assertEqual(m.mtp_draft_layer0_skipped_backpressure, 1)
    ...
    def test_recommendations_runtime_trace_mtp_ablation_supports_synthetic_mtp(self) -> None:
        ...
        trace = [
            scheduler_sim.TokenRoute(t_ms=0.0, cls=scheduler_sim.LatencyClass.INTERACTIVE, candidates=(0, 1, 2)),
            scheduler_sim.TokenRoute(t_ms=1.0, cls=scheduler_sim.LatencyClass.BATCH, candidates=(0, 1, 2)),
            scheduler_sim.TokenRoute(t_ms=2.0, cls=scheduler_sim.LatencyClass.BATCH, candidates=(0, 1, 2)),
        ]
        ...
        self.assertEqual(out.get("mtp_mode"), "synthetic")
        self.assertIn("results", out)
        self.assertIn("arrival_units_steps", out["results"])
        self.assertIn("arrival_units_output_tokens", out["results"])
        self.assertIn("mtp_off", out["results"]["arrival_units_steps"]["variants"])
