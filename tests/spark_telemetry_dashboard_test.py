...
class SparkTelemetryDashboardTest(unittest.TestCase):
    ...
    def test_fresh_cached_fetch_error_is_not_shown_as_stale(self):
        payload = {
            "updated_iso": "2026-05-26T00:00:00+00:00",
            "updated_unix": 1,
            "nodes": {"spark3": {"sample_count": 2, "last_sample_age_s": 5, "fetch_error": "ssh timed out", "last_gpu_util_pct": 0}},
        }
        with tempfile.TemporaryDirectory() as tmp:
            path = Path(tmp) / "summary.json"
            path.write_text(json.dumps(payload), encoding="utf-8")
        ...
        self.assertEqual(snap["nodes"][0]["state_label"], "idle")
        self.assertEqual(snap["nodes"][0]["fetch_error"], "")
