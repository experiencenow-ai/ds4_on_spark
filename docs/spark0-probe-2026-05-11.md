...
```bash
(REDACT=1 SPARK_PROBE_FACTS=1 SPARK_KNOWN_HOSTS_PER_HOST=1 DS4_GIT_DIR=.git-codex DS4_GIT_WORK_TREE=. ./scripts/spark_probe.sh spark0@aitopatom-9ab9.local spark1.local || true) | tee /private/tmp/ds4_spark01_probe_facts_redacted_2026-05-11T0026Z.txt
```

Notes:

- Output is redacted (`REDACT=1`) to remove IPv4/IPv6/MAC addresses and GPU UUID tokens.
- Facts-only mode (`SPARK_PROBE_FACTS=1`) implies summary mode and trims variable runtime sections while keeping stable identity + CUDA/toolchain + GPU inventory + disk model/size facts.

## Probe Output (Redacted)

```text
== local meta ==
Mon May 11 00:24:55 UTC 2026
git: 5c9c4f9
...
Mem:           119Gi       4.3Gi        57Gi        57Mi        59Gi       115Gi
Swap:           15Gi       572Mi        15Gi
