...
- `ds4_log_ring_drain_format`: Drains the ring into a caller buffer as newline-delimited `level: msg` lines; sets `out_truncated=1` if the buffer fills before all entries can be written.
