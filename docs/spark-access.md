...
- `scripts/spark_probe.sh` optional toggles:
  ...
  - `NVCC_ARCH=sm_121` force the `nvcc` runtime probe to compile for a specific GPU arch (defaults to deriving from `nvidia-smi` compute capability when available)
