...
For lightweight diagnostics, the CUDA wrapper also exposes `ds4_cuda_device_count`, `ds4_cuda_device_info` (device 0), and `ds4_cuda_get_device`/`ds4_cuda_set_device`.
...
For async sequencing (future kernels), the wrapper also exposes `ds4_cuda_stream_*`, `ds4_cuda_event_*`, and async transfer helpers (`ds4_cuda_memset_async`, `ds4_cuda_memcpy_*_async`) (stubbed to `DS4_CUDA_ERR_DISABLED` in CPU-only builds).
