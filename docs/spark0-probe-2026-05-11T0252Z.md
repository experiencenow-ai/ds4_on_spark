...
```bash
(REDACT=1 SPARK_PROBE_SUMMARY=1 SPARK_KNOWN_HOSTS_PER_HOST=1 DS4_GIT_DIR=.codex_git DS4_GIT_WORK_TREE=. SPARK_SSH_USER=spark0 ./scripts/spark_probe.sh aitopatom-9ab9.local spark1.local || true) | tee /private/tmp/ds4_spark01_probe_summary_redacted_2026-05-11T0252Z.txt
```

Notes:

- Output is redacted (`REDACT=1`) to remove IPv4/IPv6/MAC addresses and GPU UUID tokens.
- Summary mode (`SPARK_PROBE_SUMMARY=1`) keeps the CUDA/toolchain + PCIe link state + network/storage sections while suppressing larger diagnostic dumps.
- Spark1 is still expected to fail in this run; `|| true` preserves the Spark0 output.
- This snapshot records a PCIe link state of Gen1 x1 (`current_link_speed: 2.5 GT/s`, `current_link_width: 1`) with the driver reporting `hostmax=5`/`width.max=16`; treat this as a bring-up investigation item.

## Probe Output (Redacted)

```text
== local meta ==
Mon May 11 02:54:45 UTC 2026
git: aa03e9b
...
0, 0000000F:01:00.0, [N/A], 12.72, 2398, 2398, [N/A], 0, 0
