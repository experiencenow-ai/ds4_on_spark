...
The summary includes `task_queue_wait_ms_p{50,95}` (overall), plus per-class and per-MTP-phase variants (e.g. `task_queue_wait_ms_p95_interactive`, `task_queue_wait_ms_p95_mtp_verify`) to spot starvation and draft-induced verify delays quickly.
...
To model **hypothetical DeepSeek MTP** on a target-only routing trace (no `mtp_accept_len` / `accepted_mtp` / `rejected_mtp` counters), run the runtime-trace ablation harness with an explicit synthetic accept model. Prefer inspecting `arrival_units_output_tokens` so the comparison holds output-token demand roughly constant:

```bash
python3 sim/scheduler/recommendations.py --trace-jsonl /path/to/route.jsonl --trace-input-format runtime --trace-non-route skip --mtp-draft-len 2 --mtp-accept-prob 0.7 --mtp-accept-decay 0.6
```
