...
Required fields:
- `schema`: `"ds4_pairwise_judge_record_v1"`, `"ds4_pairwise_judge_record_v2"`, or `"ds4_pairwise_judge_record_v3"`
...
- `schema="ds4_pairwise_judge_record_v3"` is **strict-by-schema**: it always enforces strict decision consistency (margin/score constraints + compact tags) during validation, without needing `--strict`.
- `judge_model`: string
...
- `fixtures/judge-elo/schemas/ds4_pairwise_judge_record_v3.schema.json` (strict-by-schema + tokens/latency required)
...
To emit `schema="ds4_pairwise_judge_record_v3"` (strict-by-schema + tokens/latency required), use `--record-schema v3` (budget fields required; strict decision constraints are always enforced):
