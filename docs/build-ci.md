...
When `DS4_ENABLE_TESTS=ON`, CTest also runs a small build-matrix smoke test (`ds4_build_matrix`) that configures/builds a few option combinations (library-only, CLI-only, tests-without-CLI, and tests+CLI with sanitizers) in CPU-only mode.
