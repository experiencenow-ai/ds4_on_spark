...
| --- | --- | --- | --- | ---: | --- | --- |
| A | `ssweens/llama.cpp-deepseek-v4` (`bb648b31e137a44b1ee72907e20ad8fb1f21d644`) | `ssweens/DeepSeek-V4-Flash-GGUF-YMMV` IQ1_M (`836cfa0faa2888e4a8ca91310d1adec9a66b66ca`) | MIT / MIT | 62.9 | Plausible | Most headroom for KV/cache among pinned GGUF candidates; runtime is a DeepSeek-V4-capable llama.cpp fork (CPU/CUDA/ROCm/Vulkan claimed). |
| B | `ssweens/llama.cpp-deepseek-v4` (`bb648b31e137a44b1ee72907e20ad8fb1f21d644`) | `ssweens/DeepSeek-V4-Flash-GGUF-YMMV` IQ2_XXS shards (`836cfa0faa2888e4a8ca91310d1adec9a66b66ca`) | MIT / MIT | 72.6 | Plausible | Sharded; requires the same V4-capable llama.cpp fork; verify shard auto-load behavior + KV/cache sizing. |
| J | `ssweens/llama.cpp-deepseek-v4` (`bb648b31e137a44b1ee72907e20ad8fb1f21d644`) | `ssweens/DeepSeek-V4-Flash-GGUF-YMMV` IQ3_XXS shards (`836cfa0faa2888e4a8ca91310d1adec9a66b66ca`) | MIT / MIT | 104.2 | Plausible but tight | Leaves limited headroom for KV/cache on Spark0 baseline; keep as a reference in case IQ1/2 variants regress. |
...
| D | `kamnxt/llama.cpp-deepseek-v4-flash-cuda-spark` (`9222e55c13c965ccb7e9104fda58796edd84a732`) | `teamblobfish/DeepSeek-V4-Flash-GGUF` IQ2_XXS-XL shards (`8224b059b3bd5f5846538a804298412cc971ace3`) | MIT / MIT | 73.1 | Plausible | Most Spark-targeted CUDA fork in manifest; shard auto-load behavior is documented upstream (point at shard 00001). |
| E | `cchuter/llama.cpp` `feat/v4-port` (`19b63dc368dfef6db6783e5ba3143927b7ed1c96`) | `teamblobfish/DeepSeek-V4-Flash-GGUF` IQ2_XS-XL shards (`8224b059b3bd5f5846538a804298412cc971ace3`) | MIT / MIT | 81.0 | Plausible | Popular “V4 loader + kernels” fork referenced by teamblobfish; more headroom than ~90–100 GiB options. |
| I | `cchuter/llama.cpp` `feat/v4-port` (`19b63dc368dfef6db6783e5ba3143927b7ed1c96`) | `teamblobfish/DeepSeek-V4-Flash-GGUF` IQ1_S-XL shards (`8224b059b3bd5f5846538a804298412cc971ace3`) | MIT / MIT | 57.3 | Plausible | Smaller teamblobfish quant (sharded) with the most KV/cache headroom among pinned GGUF options in that repo. |
| K | `cchuter/llama.cpp` `feat/v4-port` (`19b63dc368dfef6db6783e5ba3143927b7ed1c96`) | `teamblobfish/DeepSeek-V4-Flash-GGUF` IQ1_M-XL shards (`8224b059b3bd5f5846538a804298412cc971ace3`) | MIT / MIT | 63.2 | Plausible | Still significant headroom for KV/cache vs ~70–100 GiB candidates; sharded artifact. |
