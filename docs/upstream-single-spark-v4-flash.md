...
| --- | --- | --- | --- | ---: | --- | --- |
| A | `ssweens/llama.cpp-deepseek-v4` (`5b36105bae79a7127b39780c77ed22265d963f9a`) | `ssweens/DeepSeek-V4-Flash-GGUF-YMMV` IQ1_M (`e559a88dbceeed0e531257bbcdd66c3cc7359ddd`) | MIT / MIT | 62.9 | Plausible | Most headroom for KV/cache among pinned GGUF candidates; runtime is a DeepSeek-V4-capable llama.cpp fork (CPU/CUDA/ROCm/Vulkan claimed). |
| B | `ssweens/llama.cpp-deepseek-v4` (`5b36105bae79a7127b39780c77ed22265d963f9a`) | `ssweens/DeepSeek-V4-Flash-GGUF-YMMV` IQ2_XXS shards (`e559a88dbceeed0e531257bbcdd66c3cc7359ddd`) | MIT / MIT | 72.6 | Plausible | Sharded; requires the same V4-capable llama.cpp fork; verify shard auto-load behavior + KV/cache sizing. |
| J | `ssweens/llama.cpp-deepseek-v4` (`5b36105bae79a7127b39780c77ed22265d963f9a`) | `ssweens/DeepSeek-V4-Flash-GGUF-YMMV` IQ3_XXS shards (`e559a88dbceeed0e531257bbcdd66c3cc7359ddd`) | MIT / MIT | 104.2 | Plausible but tight | Leaves limited headroom for KV/cache on Spark0 baseline; keep as a reference in case IQ1/2 variants regress. |
| C | `kamnxt/llama.cpp-deepseek-v4-flash-cuda-spark` (`9222e55c13c965ccb7e9104fda58796edd84a732`) | `antirez/deepseek-v4-gguf` IQ2XXS (`c566ab6d7c696ddd0c7f124e115228af1a326824`) | MIT / MIT | 80.8 | Plausible | Reported running on a single GB10-class Spark: `https://forums.developer.nvidia.com/t/deepseek-v4-flash-iq2xxs-on-a-single-gb10/368970` |
| D | `kamnxt/llama.cpp-deepseek-v4-flash-cuda-spark` (`9222e55c13c965ccb7e9104fda58796edd84a732`) | `teamblobfish/DeepSeek-V4-Flash-GGUF` IQ2_XXS-XL shards (`3efdad27c080100655fe90b4b9b39224d0e300b4`) | MIT / MIT | 73.1 | Plausible | Most Spark-targeted CUDA fork in manifest; shard auto-load behavior is documented upstream (point at shard 00001). |
| E | `cchuter/llama.cpp` `feat/v4-port` (`19b63dc368dfef6db6783e5ba3143927b7ed1c96`) | `teamblobfish/DeepSeek-V4-Flash-GGUF` IQ2_XS-XL shards (`3efdad27c080100655fe90b4b9b39224d0e300b4`) | MIT / MIT | 81.0 | Plausible | Popular “V4 loader + kernels” fork referenced by teamblobfish; more headroom than ~90–100 GiB options. |
| I | `cchuter/llama.cpp` `feat/v4-port` (`19b63dc368dfef6db6783e5ba3143927b7ed1c96`) | `teamblobfish/DeepSeek-V4-Flash-GGUF` IQ1_S-XL shards (`3efdad27c080100655fe90b4b9b39224d0e300b4`) | MIT / MIT | 57.3 | Plausible | Smaller teamblobfish quant (sharded) with the most KV/cache headroom among pinned GGUF options in that repo. |
...
| H | `antirez/ds4` (`ed5d30dba0a1ef0f7fb863270df8f11df13653a4`) | `antirez/deepseek-v4-gguf` IQ2XXS (`c566ab6d7c696ddd0c7f124e115228af1a326824`) | MIT / MIT | 80.8 | Plausible | Upstream `ds4` now includes a CUDA path and reports DGX Spark GB10 q2 `7047` prompt tokens at `343.81` prefill t/s + `13.75` generation t/s. Reproduce locally before treating as our result. |
...
| --- | --- | --- | ---: | --- | --- |
| L | `bleysg/DeepSeek-V4-Flash-IQ2XXS-Q2K-FP8-120GB-target` (`0cb3642b466e93bc30d83ff3f9afb122914e9645`) | MIT | 82.34 | Plausible | Pinned bring-up path is vLLM + `Entrpi/ds4-spark-vllm` overlay (`--quantization deepseek_v4_hybrid_iq2`); still requires a human-approved fixture + on-hardware validation. |
