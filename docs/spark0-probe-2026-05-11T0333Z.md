...
```bash
(REDACT=1 SPARK_PROBE_SUMMARY=1 SPARK_KNOWN_HOSTS_PER_HOST=1 DS4_GIT_DIR=.codex_git DS4_GIT_WORK_TREE=. SPARK_SSH_USER=spark0 ./scripts/spark_probe.sh aitopatom-9ab9.local spark1.local || true) | tee /private/tmp/ds4_spark01_probe_summary_redacted_2026-05-11T0333Z.txt
```
...
- This snapshot still reports a PCIe link state of Gen1 x1 (`pcie.link.gen.current=1`, `pcie.link.width.current=1`), with both driver and sysfs warnings about max-gen reporting.

## Probe Output (Redacted)

```text
== local meta ==
Mon May 11 03:33:27 UTC 2026
git: f6d1b2c
...
Mem:           119Gi       4.2Gi        57Gi        57Mi        59Gi       115Gi
...
0, 0000000F:01:00.0, [N/A], 12.43, 2398, 2398, [N/A], 0, 0
