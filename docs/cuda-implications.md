...
- The current CUDA 13.0 (`V13.0.88`) cuBLASLt stack on Spark0 returns `CUBLAS_STATUS_NOT_SUPPORTED` for the E5M2 smoke probe (`cuda_cublaslt_fp8_e5m2_smoke`), which may matter for DeepGEMM paths that use E5M2 inputs.
