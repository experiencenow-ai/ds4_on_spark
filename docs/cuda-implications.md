...
- `scripts/cuda_probe_micro_spark0.sh` is the fastest no-transfer preflight when you want a single SSH session to capture CUDA 13 `nvcc` support for `sm_121`, a one-line `schema=4` device summary, and a best-effort kernel launch+sync smoke
