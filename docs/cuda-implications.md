...
- `scripts/cuda_probe_sm121_compile_probes_minimal_spark0.sh` is a no-transfer compile-only gate that records which `nvcc` flag spellings work for `sm_121` / `compute_121` (including `--gpu-architecture` / `--gpu-code` forms), plus C++20 + `--extended-lambda` / `--expt-relaxed-constexpr` acceptance (DeepGEMM/CUTLASS-style toolchain gate); `scripts/cuda_probe_minimal_gates_spark0.sh` runs it by default (set `WITH_SM121_COMPILE_PROBES_MINIMAL=0` to skip)
