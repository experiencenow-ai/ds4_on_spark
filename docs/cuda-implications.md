...
- `scripts/cuda_probe_micro_spark0.sh` is the fastest no-transfer preflight when you want a single SSH session to capture CUDA 13 `nvcc` support for `sm_121`, a one-line `schema=4` device summary, and a best-effort kernel launch+sync smoke (it also records best-effort `sm_121a` / `sm_121f` alias acceptance and a best-effort `__cluster_dims__(2,1,1)` compile check)
