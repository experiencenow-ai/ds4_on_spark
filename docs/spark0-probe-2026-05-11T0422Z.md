...
```bash
(REDACT=1 SPARK_PROBE_SUMMARY=1 SPARK_KNOWN_HOSTS_PER_HOST=1 DS4_GIT_DIR=.codex_git DS4_GIT_WORK_TREE=. SPARK_SSH_USER=spark0 ./scripts/spark_probe.sh aitopatom-9ab9.local spark1.local || true) | tee /private/tmp/ds4_spark01_probe_summary_redacted_2026-05-11T0422Z.txt
```

Notes:

- Output is redacted (`REDACT=1`) to remove IPv4/IPv6/MAC addresses and GPU UUID tokens.
- Summary mode (`SPARK_PROBE_SUMMARY=1`) keeps the CUDA/toolchain + PCIe link + network/storage sections while suppressing larger diagnostic dumps.
...
Mem:           119Gi        85Gi       1.0Gi        98Mi        34Gi        34Gi
Swap:           15Gi       784Mi        15Gi
...
0, 0000000F:01:00.0, [N/A], 14.04, 2398, 2398, [N/A], 1, 0
...
nvidia              14675968  216 nvidia_uvm,nvidia_modeset
