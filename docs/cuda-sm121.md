...
Observed on Spark0 (2026-05-11 / CUDA 13.0 `V13.0.88`): `nvcc --list-gpu-code` includes `sm_121` but does not list `sm_121a` or `sm_121f`; best-effort compile-only `-arch=sm_121a` and `-arch=sm_121f` both succeed, best-effort compile-only `--gpu-architecture=sm_121a` / `sm_121f` also succeeds, and best-effort build+run via `scripts/cuda_probe_nvcc_minimal_spark0.sh` also succeeds for both.
