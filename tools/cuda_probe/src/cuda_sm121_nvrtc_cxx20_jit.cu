...
int main(int argc,char **argv)
{
	...
	"extern \"C\" __device__ consteval unsigned int nvrtc_cxx20_magic_u32(void)\n"
	"{\n"
	"    return(0x12345678u + 2u);\n"
	"}\n"
	"extern \"C\" __global__ void nvrtc_cxx20_add_u32(unsigned int *out)\n"
	"{\n"
	"    int i = (int)threadIdx.x + ((int)blockIdx.x * (int)blockDim.x);\n"
	"    if ( i == 0 ) out[0] = nvrtc_cxx20_magic_u32();\n"
	"}\n";
	const char *opts[8];
	char ptx_header[160];
	nvrtcProgram prog = 0;
	...
	char *ptx = 0,*log = 0;
	int32_t nvrtc_major = 0,nvrtc_minor = 0,num_opts = 0,rc = 0;
	int32_t driver_v = 0;
	...
	opts[num_opts++] = "--std=c++20";
	opts[num_opts++] = "--gpu-architecture=compute_121";
	opts[num_opts++] = "--fmad=false";
	rc = nvrtc_check(nvrtcCreateProgram(&prog,src,"cuda_sm121_nvrtc_cxx20_jit.cu",0,0,0),-3,"nvrtcCreateProgram");
	if ( rc != 0 )
		return(rc);
	...
	rc = cu_check(cuModuleGetFunction(&fn,mod,"nvrtc_cxx20_add_u32"),-13,"cuModuleGetFunction");
	if ( rc != 0 )
	{
		cuModuleUnload(mod);
		cuCtxDestroy(ctx);
	...
	if ( rc != 0 )
		return(rc);
	printf("nvrtc_cxx20_jit ok out=0x%08x\n",out);
	return(0);
}
