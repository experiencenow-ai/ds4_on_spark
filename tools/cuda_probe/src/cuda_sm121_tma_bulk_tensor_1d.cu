...
__global__ void tma_bulk_tensor_1d_u8(const CUtensorMap *tensor_map,uint8_t *out)
{
...
#pragma nv_diag_suppress static_var_with_dynamic_init
	...
	cuda::device::experimental::cp_async_bulk_tensor_1d_global_to_shared(sh,tensor_map,0,barrier);
	barrier.arrive_and_wait();
	for (i=0; i<128; i++)
		out[i] = sh[i];
}

static int32_t encode_tensor_map_1d_u8(CUtensorMap *out,void *global_addr,uint32_t bytes)
{
	CUresult res;
	cuuint64_t dim = 0;
	cuuint32_t box = 0,stride = 0;
	if ( out == 0 || global_addr == 0 )
		return(-1001);
	if ( bytes == 0 )
		return(-1002);
	dim = (cuuint64_t)bytes;
	box = bytes;
	stride = 1;
	res = cuTensorMapEncodeTiled(out,CU_TENSOR_MAP_DATA_TYPE_UINT8,(cuuint32_t)1,global_addr,&dim,&dim,&box,&stride,CU_TENSOR_MAP_INTERLEAVE_NONE,CU_TENSOR_MAP_SWIZZLE_NONE,CU_TENSOR_MAP_L2_PROMOTION_NONE,CU_TENSOR_MAP_FLOAT_OOB_FILL_NONE);
	if ( res != CUDA_SUCCESS )
	{
...
int main(int argc,char **argv)
{
	...
	rc = encode_tensor_map_1d_u8(&h_map,(void *)d_in,(uint32_t)n);
	if ( rc != 0 )
	{
		cudaFree(d_map);
		cudaFree(d_out);
	...
	tma_bulk_tensor_1d_u8<<<1,32>>>(d_map,d_out);
	...
	printf("tma_bulk_tensor_1d rc=%d out0=%02x out127=%02x\n",rc,h_out[0],h_out[127]);
	if ( rc != 0 )
		return(-11);
	return(0);
}
