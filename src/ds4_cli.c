...
static void ds4_cli_usage(FILE *fp,const char *argv0)
{
	...
	if ( argv0 == 0 )
		argv0 = "ds4_cli";
	fprintf(fp,"usage: %s [--config PATH] [--log-level LVL] [--enable-cuda BOOL] [--cuda-device DEV] [--dump-config] [--version]\n",argv0);
...
static int32_t ds4_cli_parse_args(int32_t argc,char **argv,const char **cfg_path,const char **log_level,const char **enable_cuda,const char **cuda_device,int32_t *dump_cfg,int32_t *print_ver)
{
	int32_t i;
	const char *a;
	if ( cfg_path == 0 )
...
int main(int argc,char **argv)
{
	...
	enable_cuda = 0;
	cuda_device = 0;
	err = ds4_cli_parse_args((int32_t)argc,argv,&cfg_path,&log_level,&enable_cuda,&cuda_device,&dump_cfg,&print_ver);
	if ( err != 0 )
	{
		if ( err > 0 )
		{
